...
[v10 최종 수정] "nameKo=이름"으로 단건 질의하던 방식을 폐기했다.
  실제로 API를 호출해 확인한 결과, 
  nameKo 파라미터는 공식 문서(mpva.go.kr)에는 나와 있지만 서버가 실제로는 무시한다 
  — nameKo 값을 뭘 넣어도 TOTAL_COUNT=19059,
  그리고 항상 가나다순 "가네코 후미코, 가재연, 가재창..."으로 시작하는 첫 페이지가 그대로 돌아온다
  (nameKo가 없을 때와 결과가 완전히 동일).
  
  즉 서버 측 이름 필터는 작동하지 않는다.

  따라서 이번 버전은 이름별 단건 질의(20회) 대신,  
  nCountPerPage=50으로 전체 목록(약 19,059명 ÷ 50 ≈ 382페이지)을 처음부터 끝까지 순회하면서,
  각 페이지에서 v9.5 인물 20명의 NAME_KO와 정확히 일치하는 레코드만
  그때그때 수집하는 방식으로 바꿨다. 
  이렇게 하면 서버의 이름 필터가 작동하든 안 하든 결과가 항상 정확하다
  (전체를 다 보므로 필터링을 서버에 의존하지 않는다)

  관련 자세한 설명: v6/v8_with_gonghun이 원래 시도했던 것도 전체 순회 방식이었고, 
  그때 실패한  이유는 응답 필드를 rec["성명"]/rec["name"]으로 읽어서였다
  (실제 필드명은 NAME_KO).
  이번 스크립트는 필드명은 그때 이미 바로잡힌 것을 그대로 쓰고, 
  "전체 순회" 방식으로 되돌리되 페이지 수를 하드코딩하지 않고 첫 응답의 TOTAL_COUNT/
  COUNT_PER_PAGE로 동적으로 계산한다.

  전체 순회이므로 요청 수가 많다(약 382회). 서
  버 부담을 줄이기 위해 요청 사이 0.3초 간격을 유지하며, 
  사람이 지켜보지 않아도 완주할 수 있도록 각 페이지 요청은 실패 시 최대 3회 재시도하고, 
  중간에 중단돼도 이어서 재개할 수 있도록 20페이지마다 중간 저장(checkpoint)을 남긴다.

참고> 브라우저 대신 로컬 스크립트를 사용한 이유.
  - 이 API(e-gonghun.mpva.go.kr)는 HTTP만 제공한다. 
    그런데 사이트는 HTTPS로 배포돼 있어서, 
    브라우저가 "혼합 콘텐츠(mixed content)"로 요청 자체를 막는다
    (코드를 아무리 고쳐도 브라우저 정책이라 우회 불가 
    — 실제로 배포 후 전원 "Failed to fetch"로 확인됨).
  - 무료 공개 프록시(allorigins, codetabs)로 우회를 시도했지만 
    타임아웃/실패가 반복됐다 — 정부 사이트가 프록시발 트래픽을 막거나 응답이 느린 것으로 보인다.
  - Python의 urllib은 브라우저처럼 http를 https로 자동 승격하지 않으므로, 
    같은 API를 로컬에서는 안정적으로 호출할 수 있다.
  - 그래서 "브라우저에서 매번 실시간 호출" 대신 "로컬에서 한 번 조회해 스냅샷을 만들고, 
    사이트는 그 정적 파일만 읽는" 방식으로 바꿨다.

사용법:
  python3 gonghun_match.py
  (전체 순회라 수 분 정도 걸릴 수 있습니다. 
   완료되면 site/ontology/versions/data/gonghun_snapshot.json 파일이 생성/갱신됩니다.
