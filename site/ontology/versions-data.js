...
const ONTOLOGY_VERSIONS = [
  {
    ...
    changelog:
      ...
      "세 번째 탭으로공훈전자사료관 연동을 다시 추가하되 v6/v8_with_gonghun의 근본 버그를 고침:  " +
      "(1) 응답 필드를 rec['성명']||rec['name']으로 읽어 항상 빈 문자열이었던 것을 실제 필드명 " +
      "NAME_KO로 정정 — 이게 지금까지 0명 매칭의 진짜 원인. (2) '이름 필터 파라미터 " +
  ...
  },
  {
    ...
    label: "의미론 재검토 표시 추가",
    changelog:
      "v9.5 데이터(core 34/hold 5)는 전혀 바꾸지 않음. " +
      " v9.5 설명서·검증서(이 얘기는 온톨로지 저장소 참고) 검토에서," +
      "date_status='not_applicable'인 참여 Claim 6건 중 CLM-024('합류하였다')" +
      "·CLM-027-04('이끌었다')는 동작 동사를 쓰고 있어 다른 4건(지위·성격 서술)과 달리 " +
      "not_directly_verified가 더 적합할 수 있다고 재검토했으나,"+
      "정책(참여 Claim은 개별 날짜 불요 vs 날짜축은 있으나 미검증) 자체가 아직 확정되지 않아 " +
      "데이터는 그대로 두고 지도·관계망·상세패널에만 '🔍 의미론 재검토 권고' 표시를 추가." +
      "core/hold(데이터 상태)와 review(시각화상의 검토 의견)를 섞지 않기 위한 방안이었으며," +
      "재검토 간선 2건이 정확히 표시되는 것을 배포 전 실행 검증함.",
  ...
  },
  {
    ...
    label: "Claim 기반 근거모델",
    changelog:
      "v8까지의 단일 Event 스키마에서 한 걸음 더 나아가, 사실 하나하나를 Claim 단위로 분리하고 " +
      "각 Claim에 Source(출처)·Evidence(근거)·claim_status·date_status·" +
      ...
      "관계망(vis-network) 2탭 구조(v6과 동일한 '기존 스타일')로 제작  " +
      "— hold Claim은 점선·주황색으로 core와 시각적으로 구분." +
      "jsdom+실제 d3/vis-network로 노드 40개·간선 31개(hold 5개 포함)가 "+
      "정확히 생성되는 것을 배포 전에 실행 검증함.",
  ...
  },
  {
    ...
    changelog:
      "v1.html의 D3.js force-graph UI(필터 버튼·검색창·범례·상세패널)를 그대로 두고, " +
      "하드코딩 데이터 대신 v8.ttl을 브라우저에서 실시간 파싱해 채움. " +
      "Event 중심 스키마를 이항관계 2개로 접어 v1과 동일한 그래프 형태로 표시(노드 44개·간선 48개)." +
      "v1은 최린·정춘수·박희도 3인을 '친일 전향'으로 표시했으나, v8 온톨로지엔 현재 최린만 :전향 이벤트가 명시돼 있어" +
      "이 그래프는 그 실제 데이터 내용을 있는 그대로 반영함(정춘수·박희도는 일반 인물 노드로 표시).",
  ...
  },
  {
    ...
    changelog:
      ...
      "v5 인물 30명과 대조. API 호출 경우 직접 시도했고, API 호출이 잘 안 되어 클로드 코드 사용." +
      "정부 사이트 특성상 CORS로 막힐 가능성이 있었고, 이 때문에 실패 시 원인과 Python 스크립트 대안을 " +
      "화면에 그대로 제시. [로컬 스크립트는 9월에야 시도함] GIS·네트워크 탭 로직은 v6과 동일.",
  ...
  },
  {
    ...
    changelog:
      ...
      "구현. v5.ttl을 브라우저에서 실시간 파싱해 (1) Leaflet 기반 GIS 지도 — 조직 소재지 POI " +
  ...
  },
  {
    ...
    changelog:
      "Prof.김바로, 「제도-인사 온톨로지 설계」(한국학중앙연구원 한국학대학원 박사논문)의 학술모델·추정모델·" +
